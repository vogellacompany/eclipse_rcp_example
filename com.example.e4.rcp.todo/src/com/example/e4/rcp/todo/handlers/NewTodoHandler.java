...
public class NewTodoHandler {
	@Execute
	public void execute(Shell shell, ITodoService model, @Translation Messages messages) {
		// use -1 to indicate a not existing id
		Todo todo = new Todo(-1);
		todo.setDueDate(new Date());
		WizardDialog dialog = new WizardDialog(shell, new TodoWizard(todo, messages ));
